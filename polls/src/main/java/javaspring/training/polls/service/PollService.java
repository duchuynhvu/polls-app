package javaspring.training.polls.service;
...
import javaspring.training.polls.exception.BadRequestException;
import javaspring.training.polls.exception.ResourceNotFoundException;
import javaspring.training.polls.model.Choice;
import javaspring.training.polls.model.ChoiceVoteCount;
import javaspring.training.polls.model.Poll;
import javaspring.training.polls.model.User;
import javaspring.training.polls.model.Vote;
...
import javaspring.training.polls.util.ModelMapper;
...
public class PollService {
    ...
    public PagedResponse<PollResponse> getAllPolls(UserPrincipal currentUser,
        ...
        if (polls.getNumberOfElements() == 0) {
            return new PagedResponse<>(Collections.emptyList(),
                    ...
                    polls.getTotalElements(), polls.getTotalPages(),
                    polls.isLast());
        }

        // Map Polls to PollResponses containing vote counts and poll creator
        // details
        ...
        List<PollResponse> pollResponses = polls
                .map(poll -> ModelMapper
                        .mapPollToPollResponse(poll, choiceVoteCountMap,
                                creatorMap.get(poll.getCreatedBy()),
                                pollUserVoteMap == null
                                        ? null
                                        : pollUserVoteMap.getOrDefault(
                                                poll.getId(), null)))
                .getContent();
    ...
    public PagedResponse<PollResponse> getPollsCreatedBy(String username,
        ...
        List<PollResponse> pollResponse = polls
                .map(poll -> ModelMapper
                        .mapPollToPollResponse(poll, choiceVoteCountMap, user,
                                pollUserVoteMap == null
                                        ? null
                                        : pollUserVoteMap.getOrDefault(
                                                poll.getId(), null)))
        ...
        return new PagedResponse<>(pollResponse, polls.getNumber(),
                polls.getSize(), polls.getTotalElements(),
                polls.getTotalPages(), polls.isLast());
    }
    ...
    public PagedResponse<PollResponse> getPollsVotedBy(String username,
        ...
        Pageable pageable = PageRequest.of(page, size, Direction.DESC,
                CREATED_AT);
        Page<Long> userVotedPollIds = voteRepository
                .findVotedPollIdsByUserId(user.getId(), pageable);
    ...
    public PollResponse getPollById(Long pollId, UserPrincipal currentUser) {
        ...
        List<ChoiceVoteCount> votes = voteRepository
                .countByPollIdGroupByChoiceId(pollId);

        Map<Long, Long> choiceVotesMap = votes.stream()
                .collect(Collectors.toMap(ChoiceVoteCount::getChoiceId,
    ...
    public PollResponse castVoteAndGetUpdatedPoll(Long pollId,
            VoteRequest voteRequest, UserPrincipal currentUser) {
        Poll poll = pollRepository.findById(pollId).orElseThrow(
                () -> new ResourceNotFoundException("Poll", "id", pollId));
        ...
        Choice selectedChoice = poll.getChoices().stream()
                .filter(choice -> choice.getId()
                        .equals(voteRequest.getChoiceId()))
                .findFirst()
                .orElseThrow(() -> new ResourceNotFoundException("Choice", "id",
        ...
        try {
            ...
            LOGGER.info("User {} has already voted in Poll {}",
                    currentUser.getId(), pollId);
            throw new BadRequestException(
                    "Sorry! You have already cast your vote in this poll");
        }
    ...
    private Map<Long, Long> getPollUserVoteMap(UserPrincipal currentUser,
            List<Long> pollIds) {
        // Retrieve Votes done by the logged in user to the given pollIds
        Map<Long, Long> pollUserVoteMap = null;
        if (currentUser != null) {
            List<Vote> userVotes = voteRepository
